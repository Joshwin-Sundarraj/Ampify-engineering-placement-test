...
private:
    ...
    std::optional <double> findAndExtractLHS (std::string input, std::string character) const
    {
        ...
        else if (auto pos = input.find ("pi"); pos != std::string::npos && input.substr(0,2)=="pi")
    ...
    {
        if (auto pos = input.find (character); pos != std::string::npos){
            if (auto pos_pi = input.find ("pi",pos); pos_pi != std::string::npos)
              return pi;
...
public:
    ...
    {
        std::cout.precision(precision_value);
        switch (tokens.type)
        {
...
void test ()
{
    ...
    //test changed to -2.6 because 3-5.6=-2.6 not 2.6
}
